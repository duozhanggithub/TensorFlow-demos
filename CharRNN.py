...
def get_batches(arr, batch_size, n_steps):
    ...
    for n in range(0, arr.shape[1], n_steps):
        ...
        if n == arr.shape[1] - n_steps:
            ...
            y[:, :-1], y[:, -1] = arr[:, n + 1:], arr[:, 1]

        yield tf.one_hot(x, len(vocab)), y
...
def build_inputs(batch_size, num_steps):
    ...
    inputs = tf.placeholder(tf.float32, [batch_size, num_steps, len(vocab)], name='inputs')
...
def build_loss(logits, targets, lstm_size, num_classes):
    ...
    # y_reshaped =  tf.reshape(logits, [-1, num_classes])
...
class CharRNN:
    ...
    def __init__(self, num_classes, batch_size=64, num_steps=50,
        ...
        self.loss = build_loss(logits, self.targets, lstm_size, num_classes)
...
with tf.Session() as sess:
    ...
    for e in range(epochs):
        ...
        for x, y in get_batches(encoded, batch_size, num_steps):
            counter += 1
            start = time.time()
            feed = {model.inputs: x.eval(session=sess),
                    model.targets: y,
